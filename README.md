...
- **Standalone-cover spread offset** — a double-page-only toolbar toggle
  that shows page 1 alone, then pairs (2,3) (4,5)… so a lone cover doesn't
  push every facing spread one page out of alignment. Persisted; off by
  default. All spread grouping (navigation, the visible span, position
  restore) routes through one `SpreadCalculator`, so the pairing stays
...
- **Reopens last library on launch** — the folder you were browsing is
  ...
  With) takes precedence and skips the restore. **Settings → Library** has a
