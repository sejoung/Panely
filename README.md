...
- **Persistent zip-in-zip cache** — extracted nested archives are content-
  ...
  edits bump mtime → new key → automatic re-extraction.
...
**289 tests across 46 suites** cover:
...
│   └── SnapshotGalleryTests.swift      # 11 manual scenarios
...
├── screenshots/                        # 11 PNGs generated by SnapshotGalleryTests
...
- **CI** runs on every push/PR (skips `**/*.md` and `docs/**`), builds
  Debug with ad-hoc signing, runs all 289 tests, and uploads no artifacts —
  storage footprint is essentially zero. `SnapshotGalleryTests` is
  explicitly `-skip-testing`'d in CI (and the release script) because it
  has no assertions; regenerate the manual's PNGs with
